...
const CavitationRenderer: React.FC<CavitationRendererProps> = ({ onEvent, gamePhase }) => {
  // Phase management
      ...
      setBubbles(prev => prev.map(b => {
        ...
        if (b.collapsing) {
          ...
          if (newRadius < 1) {
            ...
            playSound(80 + Math.random() * 40, 0.05, 'sawtooth', 0.1);
  ...
  const renderPlay = () => (
    <div style={{ padding: isMobile ? '16px' : '24px' }}>
      ...
      {damageLevel > 30 && renderBottomBar(() => goToPhase('review'))}
    </div>
  );
