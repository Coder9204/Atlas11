...
const CoupledPendulumsRenderer: React.FC<CoupledPendulumsRendererProps> = ({
  ...
  // Real-world applications of coupled oscillator physics
  const realWorldApps = [
    {
      icon: '🏗️',
      title: 'Building Tuned Mass Dampers',
      short: 'Structural vibration control',
      tagline: 'Protecting Skyscrapers from Catastrophic Oscillations',
      description: 'Tuned mass dampers (TMDs) are massive pendulum systems installed in skyscrapers to counteract wind-induced and seismic oscillations. The famous 730-ton TMD in Taipei 101 uses coupled oscillator physics to prevent building sway that could cause structural damage or occupant discomfort.',
      connection: 'Just like our coupled pendulums transfer energy back and forth, a TMD is tuned to oscillate out of phase with the building. When the building sways one direction, the TMD swings opposite, transferring the building\'s kinetic energy into the damper system where it dissipates.',
      howItWorks: 'The TMD is a massive pendulum (often 300-700 tons) suspended near the top of the building. Its natural frequency is tuned to match the building\'s dominant sway frequency. When wind or earthquakes excite the building, energy transfers to the TMD through coupled oscillation. Hydraulic dampers then absorb this energy, reducing building motion by 30-50%.',
      stats: [
        { value: '730 tons', label: 'Taipei 101 TMD mass', icon: '⚖️' },
        { value: '40%', label: 'Typical sway reduction', icon: '📉' },
        { value: '5.5m', label: 'Max damper swing', icon: '↔️' }
      ],
      examples: [
        'Taipei 101\'s golden orb TMD is a tourist attraction and engineering marvel',
        'Citigroup Center in NYC uses a 400-ton TMD to counter wind loads',
        'Shanghai Tower employs a 1,000-ton eddy current damper system',
        'John Hancock Tower in Boston added a TMD after problematic oscillations'
      ],
      companies: ['Motioneering', 'GERB Vibration Control', 'Taylor Devices', 'Maurer AG', 'Thornton Tomasetti'],
      futureImpact: 'Active and semi-active TMDs with real-time frequency tuning will enable adaptive response to changing conditions, making buildings safer during extreme weather events and earthquakes.',
      color: '#3b82f6'
    },
    {
      icon: '🎵',
      title: 'Metronome Synchronization',
      short: 'Physics demonstration',
      tagline: 'The Mysterious Dance of Coupled Oscillators',
      description: 'When multiple metronomes are placed on a common movable platform, they spontaneously synchronize their ticks over time - a stunning demonstration of coupled oscillator physics discovered by Christiaan Huygens in 1665 with pendulum clocks.',
      connection: 'This is exactly what you observed in our simulation! The platform acts as the coupling spring, allowing tiny amounts of energy to transfer between metronomes. Through beats and energy exchange, the metronomes gradually lock into either in-phase or anti-phase oscillation.',
      howItWorks: 'Each metronome oscillates and imparts small forces to the shared platform. The platform\'s motion feeds back to all metronomes, creating weak coupling. Over many cycles, the beat phenomenon causes energy redistribution until the system settles into a normal mode - either all metronomes swinging together (symmetric) or in alternating patterns (antisymmetric).',
      stats: [
        { value: '~1 min', label: 'Typical sync time', icon: '⏱️' },
        { value: '1665', label: 'Huygens\' discovery', icon: '📅' },
        { value: '32+', label: 'Metronomes synced (record)', icon: '🔢' }
      ],
      examples: [
        'Huygens noticed two pendulum clocks on the same shelf synchronizing their swings',
        'YouTube videos of 32+ metronomes synchronizing have millions of views',
        'Fireflies synchronizing their flashing follows similar coupled oscillator dynamics',
        'Audiences clapping in unison demonstrate spontaneous synchronization'
      ],
      companies: ['Educational Physics Suppliers', 'Science Museums Worldwide', 'MIT Physics Demos', 'Exploratorium'],
      futureImpact: 'Understanding spontaneous synchronization helps model everything from pacemaker cells in the heart to laser arrays and synchronized power grid oscillators.',
      color: '#10b981'
    },
    {
      icon: '🧠',
      title: 'Neural Oscillations',
      short: 'Brain wave coupling',
      tagline: 'How Your Brain Coordinates Through Rhythm',
      description: 'Your brain uses coupled neural oscillators to coordinate activity across regions. Different brain rhythms (alpha, beta, gamma waves) are generated by networks of neurons acting as coupled oscillators, and their synchronization enables cognition, memory, and consciousness.',
      connection: 'Just as our pendulums exchange energy through the coupling spring, brain regions exchange information through neural connections. When regions synchronize their oscillations (phase-lock), they can efficiently communicate. Disrupted coupling is linked to disorders like epilepsy and schizophrenia.',
      howItWorks: 'Populations of neurons oscillate at characteristic frequencies due to recurrent inhibitory and excitatory connections. These neural populations couple through axonal connections. When coupling strength and frequency matching align, regions phase-lock and information transfers between them - similar to our normal modes but in a vastly more complex network.',
      stats: [
        { value: '8-12 Hz', label: 'Alpha wave frequency', icon: '〰️' },
        { value: '86 billion', label: 'Neurons in human brain', icon: '🔗' },
        { value: '40 Hz', label: 'Gamma binding frequency', icon: '⚡' }
      ],
      examples: [
        'Gamma oscillations (40 Hz) bind sensory features into unified perception',
        'Alpha waves synchronize across visual cortex during attention',
        'Deep brain stimulation uses oscillation principles to treat Parkinson\'s',
        'Sleep stages are defined by characteristic coupled oscillation patterns'
      ],
      companies: ['Medtronic (DBS devices)', 'NeuroPace', 'Kernel', 'Neuralink', 'Blackrock Neurotech'],
      futureImpact: 'Brain-computer interfaces will leverage neural oscillation coupling to restore communication, treat psychiatric disorders, and potentially enhance cognitive function.',
      color: '#a855f7'
    },
    {
      icon: '⚡',
      title: 'Power Grid Synchronization',
      short: 'Electrical grid stability',
      tagline: 'Keeping Billions of Oscillators in Perfect Sync',
      description: 'The electrical power grid is the largest machine on Earth - a network of thousands of generators that must all oscillate at exactly 50 or 60 Hz in perfect synchronization. This is a massive coupled oscillator system where failure to synchronize causes blackouts.',
      connection: 'Each generator is an oscillator connected to others through power lines (the coupling). Like our pendulums, generators exchange energy through their coupling. If one generator\'s frequency drifts, the coupling forces try to pull it back - but if coupling is too weak or disturbance too strong, the system can lose synchronization catastrophically.',
      howItWorks: 'Generators are synchronous machines - their rotation rate determines AC frequency. When connected to the grid, electromagnetic forces couple them together. Any frequency deviation creates power flows that act like restoring forces in coupled oscillators. Grid operators tune coupling strength (transmission capacity) and monitor beat-like phenomena to prevent cascading failures.',
      stats: [
        { value: '60 Hz', label: 'US grid frequency', icon: '🔌' },
        { value: '±0.5%', label: 'Frequency tolerance', icon: '📏' },
        { value: '1 GW+', label: 'Large generator capacity', icon: '💡' }
      ],
      examples: [
        'The 2003 Northeast blackout affected 55 million people due to sync failure',
        'European grid briefly split into three islands in 2006 from desynchronization',
        'Wind and solar farms require synthetic inertia to participate in synchronization',
        'High-voltage DC links decouple oscillating regions to prevent cascade failures'
      ],
      companies: ['General Electric', 'Siemens Energy', 'ABB', 'National Grid', 'PJM Interconnection'],
      futureImpact: 'Smart grids with distributed energy resources will use advanced coupled oscillator control theory to maintain stability with millions of small generators instead of hundreds of large ones.',
      color: '#f59e0b'
    }
  ];
