...
export default function GeneratorStartupRenderer({
      ...
      {showPredictionFeedback && (
        <>
          ...
          <button
            onClick={() => onPhaseComplete?.()}
            style={{ WebkitTapHighlightColor: 'transparent' }}
            className="w-full py-4 px-8 rounded-2xl font-semibold text-lg bg-gradient-to-r from-orange-500 to-red-500 text-white shadow-lg"
          >
            Watch a Generator Start →
          </button>
        </>
      )}
    </div>
  ...
  const renderPlay = () => (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      >
        {hasExperimented ? 'Continue to Review →' : 'Start the generator first...'}
      </button>
    </div>
  );
  ...
  const renderTwistPlay = () => (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      >
        {hasExploredTwist ? 'Continue →' : 'Apply load to the generator...'}
      </button>
    </div>
  );
