...
# =============================================================================
# 20-PHASE ENHANCEMENT FOR PARTIAL GAMES (Leverage Existing Work)
# =============================================================================

TARGET_FILE="components/GeneratedDiagram.tsx"
DONE_FILE="work/done_20phase_partial.txt"
LOG_DIR="work/logs_20phase_partial"
...
# ---- Get timeout (shorter for partial games) ----
get_timeout() {
  local phase=$1
  if [ "$phase" -le 8 ]; then
    echo "15"
...
get_line_hint() {
  case "$1" in
    ProfitMarginRenderer) echo "24471" ;;
    FinancialRatioAnalysisRenderer) echo "25188" ;;
    FinancialStatementsRenderer) echo "23754" ;;
    InventoryValuationRenderer) echo "52540" ;;
    WorkingCapitalManagementRenderer) echo "25905" ;;
    ProfitLossRenderer) echo "30043" ;;
    BudgetForecastRenderer) echo "27338" ;;
    BreakEvenRenderer) echo "26622" ;;
    BreakEvenAnalysisRenderer) echo "26622" ;;
    AuditPrepRenderer) echo "30923" ;;
    RevenueModelsRenderer) echo "29406" ;;
    *) echo "1" ;;
  esac
}
...
process_phase() {
  ...
  echo ""
  echo "================================================================"
  echo "[$(date '+%H:%M:%S')] Processing PARTIAL: ${game} - Phase ${phase}/20"
  echo "                      ${phase_name}"
  echo "                      Timeout: ${timeout_min} min (leveraging existing)"
  echo "================================================================"
...
IMPORTANT: This game has EXISTING WORK from a previous enhancement process.
You MUST read and leverage this existing work - do NOT start from scratch.

OVERVIEW: Each game is processed in 20 phases. This game already has partial work.
...
CRITICAL RULES FOR PARTIAL GAMES:
- FIRST: Read the existing ENHANCEMENT PLAN comment block
- LEVERAGE existing 10 concepts, scenarios, and architecture
- BUILD UPON existing prediction/reveal mechanics
- PRESERVE existing state management and screen logic
- ENHANCE and COMPLETE - do NOT discard existing work
...
*** THIS GAME HAS EXISTING WORK - LEVERAGE IT ***
...
2) CAREFULLY review the existing ENHANCEMENT PLAN comment block
3) REVIEW existing state variables, prediction mechanics, and screens
4) UNDERSTAND what has already been built

EXISTING WORK TO LEVERAGE:
- The game likely has an ENHANCEMENT PLAN with 10 concepts defined
- There may be existing prediction/reveal mechanics
- There may be existing screen implementations
- USE THIS AS YOUR FOUNDATION - do not discard it

TASK FOR THIS PHASE:
1) Read and understand existing work first
2) Apply Phase ${phase} instructions, BUILDING ON existing work
3) If existing work already covers requirements, enhance and formalize it
4) Run: bash scripts/run_checks.sh
5) Output: DONE ${game} PHASE ${phase}

=== PHASE ${phase} INSTRUCTIONS ===
${phase_content}
=== END PHASE ${phase} INSTRUCTIONS ===

REMEMBER: Build upon existing work. Do not start from scratch.
  ...
  if output=$(timeout "${timeout_min}m" claude -p \
    ...
    if echo "$output" | grep -q "DONE ${game} PHASE ${phase}"; then
      ...
      if [ "$phase" -ge 9 ]; then
        ...
        git commit -m "20-phase-partial: ${game} Phase ${phase} - ${phase_name}" 2>/dev/null || true
      fi
      return 0
    else
...
# ---- Main ----
echo "=============================================================="
echo "20-PHASE: 11 PARTIAL GAMES (Leveraging Existing Work)"
echo "=============================================================="
...
echo ""
echo "=============================================================="
echo "PARTIAL GAMES COMPLETE: Done=${TOTAL_DONE}, Failed=${TOTAL_FAIL}"
echo "=============================================================="
